...
async def scrape_fertipar_data(config=None):
    ...
    if scraped_data:
        print(f"Raspagem concluída. Total de {len(scraped_data)} linhas. Filtrando por 'Situação' == 'PENDENTE' ou 'RECUSADO'...")
        filtered_data = [row for row in scraped_data if row.get('Situação') in ['PENDENTE', 'RECUSADO']]
